    ...
    <div style="position: relative; z-index: 1;">
        ...
        <div class="header-laporan">
            ...
            <div class="d-flex align-items-center gap-2">
                <a href="{{ url('/tambah-laporan') }}" class="btn btn-primary btn-sm">
                    <i class="fa fa-plus-circle me-1"></i> Tambah Laporan
                </a>

                <form action="{{ route('laporan-statistik') }}" method="GET" class="search-box">
                    <i class="fa fa-search search-icon"></i>
                    <input type="search" name="cari" class="search-input" placeholder="Cari Laporan" value="{{ request('cari') }}">
                </form>
            </div>
        </div>
        ...
        <table class="table table-bordered bg-white rounded">
            ...
            <tbody>
                @forelse ($data as $item)
                    ...
                    @endphp
                    <tr>
                        <td class="text-center">{{ $idFormatted }}</td>
                        <td>{{ $item->judul_laporan }}</td>
                        <td class="text-center">{{ \Carbon\Carbon::parse($item->tanggal_laporan)->format('d M Y') }}</td>
                        <td>{{ \Illuminate\Support\Str::limit($item->deskripsi, 50) }}</td>
                        <td class="text-center">
                            @if ($item->file_laporan)
                                <a href="{{ asset('uploads/laporan/' . $item->file_laporan) }}" 
                                   target="_blank" 
                        ...
                        <td class="text-center">
                            <div class="d-flex gap-1 justify-content-center">
                                ...
                                <form action="{{ url('/hapus-laporan/' . $item->id_laporan) }}" 
                                      method="POST" 
